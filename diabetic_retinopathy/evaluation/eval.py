...
import logging
import wandb
from evaluation.metrics import recall_score, auc_score, f1score, accuracy_score, confusion_matrix, loss_mae
...
def evaluate(model, checkpoint, ds_test, ds_info):
    ...
    if FLAGS.multi_class and not FLAGS.classification:
        mae_loss = loss_mae(true_labels, pred_probs)
        logging.info('MAE loss: {:.2f}'.format(mae_loss))
        wandb.log({"mae_loss": mae_loss})
